...
export const deleteDireccionPredeterminadaByUserID = async (req, res) => {
  const { ID_usuario } = req.params;
  ...
  try {
    ...
    const result = await pool
      .request()
      .input("ID_usuario", sql.Int, ID_usuario)
      .query(querysDireccionEnvioPredeterminada.deleteDireccionPredeterminadaByUserID);

    if (result.rowsAffected[0] === 0) {
      return res.status(404).json({ msg: "Dirección predeterminada no encontrada o ya eliminada" });
    }

    res.json({ msg: "Dirección predeterminada eliminada correctamente" });
  } catch (error) {
    console.error("Error al eliminar la dirección predeterminada:", error.message);
    res.status(500).json({ msg: "Error al eliminar la dirección predeterminada" });
  }
};
